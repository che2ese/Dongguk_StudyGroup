...
1) N의 값을 입력받음
2) 0101010 입력 (배열로 받기)
3) if 1 -> 1점 일단 받음. 그 다음 또 1이 나오면 1씩 더해서 더함 currentScore++
4) else -> 0점으로 currentScore 리셋.
*/
...
int main(void)
{
	...
	for (int i = 0; i < N; i++)
	{
		scanf("%d", &answer[i]); //01010 입력받음
	}

	int totalScore = 0;
	int currentScore = 0; //currentScore을 더해서 totalScore이 되는 것

	for (int j = 0; j < N; j++)
		if (answer[j] == 1)
		{
			currentScore++; //1씩 늘려서 더하기
			totalScore += currentScore; 
		}
		else
		{
			currentScore = 0; //0 나오면 currentScore을 초기화
		}
